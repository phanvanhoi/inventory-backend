/**
 * Batch: NHẬP ÁO KHOÁC 2026.csv → SP11 (update) + SP33+ (children SP3)
...
const csvPath =
  process.argv[2] ||
  "c:\\Users\\RemoteUser\\Downloads\\NHẬP ÁO KHOÁC 2026.csv";
const parentProductId = 3;
...
function main() {
  ...
  const sp11Name = blocks[0].name;

  const newProductRows = newBlocks
    .map((b) => `('${escapeSql(b.name)}', 'STRUCTURED', ${parentProductId}, 'Áo khoác 2026 - batch NHẬP ÁO KHOÁC', '2026-01-01 00:00:00')`)
    .join(",\n");
  ...
  const variantClones = blocks
    ...
    .map((b) => {
      ...
      return `-- SP${pid}\nINSERT INTO product_variants (product_id, size_id, gender)\nSELECT ${pid}, size_id, gender FROM product_variants WHERE product_id = 11;`;
    })
    .join("\n\n");

  const inventoryParts = blocks.map((b) => inventorySeedSql(b).sql.trim());

  const idRange = `SP11, SP33–SP${blocks[blocks.length - 1].productId}`;

  const sql = `-- =====================================================
-- PHẦN 8e: ÁO KHOÁC 2026 — BATCH NHẬP ÁO KHOÁC (${idRange})
...
UPDATE products SET product_name = '${escapeSql(sp11Name)}' WHERE product_id = 11;
...
`;
  ...
  const outPath = path.join(__dirname, "aokhoac-batch-seed.sql");
  fs.writeFileSync(outPath, sql, "utf8");

  console.log("Blocks:", blocks.length);
  console.log("SP11 name:", sp11Name);
  blocks.forEach((b) => {
